...
export default function OfficePersonalSettings() {
  const navigate = useNavigate();
  const { user } = useUser();
  const officeName = user?.unsafeMetadata?.officeName || 'Missouri City Dental';
  const ownerName = user?.lastName ? `Dr. ${user.lastName}` : 'Dr. Osagie';
  const email = user?.primaryEmailAddress?.emailAddress || 'hello@missouricitydental.com';
      ...
      <div className="card">
        <div className="profile-row" onClick={() => navigate('/account/profile-photo')}>
          <div className="profile-avatar">
            <img src={avatarUrl} alt={officeName} />
          ...
          <div className="profile-info"><div className="profile-name">{officeName}</div><div className="profile-email">{email}</div></div>
          <ChevRight />
        </div>
        ...
        <div className="row" onClick={() => navigate('/account/phone')}><div className="row-icon"><svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13 1 .37 1.97.72 2.88a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.35 1.88.59 2.88.72A2 2 0 0 1 22 16.92z" /></svg></div><div className="row-body"><div className="row-label">Office phone</div><div className="row-sub">(281) 555-0142 · Verified</div></div><ChevRight /></div>
        <div className="row" onClick={() => navigate('/account/office-owner')}><div className="row-icon"><svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg></div><div className="row-body"><div className="row-label">Primary contact</div><div className="row-sub">{ownerName}</div></div><ChevRight /></div>
        <div className="row" onClick={() => navigate('/account/home-address')}><div className="row-icon"><svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg></div><div className="row-body"><div className="row-label">Office address</div><div className="row-sub">Missouri City, TX 77489</div></div><ChevRight /></div>
      </div>
